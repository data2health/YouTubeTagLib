...
public class PlaylistPublished extends YouTubeTagLibTagSupport {
	String type = "DATE";
	...
	public int doStartTag() throws JspException {
		try {
			...
			if (!thePlaylist.commitNeeded) {
				...
				if (thePlaylist.getPublished() == null) {
					resultString = "";
				} else {
					if (pattern != null) {
						resultString = (new SimpleDateFormat(pattern)).format(thePlaylist.getPublished());
					} else if (type.equals("BOTH")) {
						resultString = DateFormat.getDateTimeInstance(formatConvert(dateStyle),formatConvert(timeStyle)).format(thePlaylist.getPublished());
					} else if (type.equals("TIME")) {
						resultString = DateFormat.getTimeInstance(formatConvert(timeStyle)).format(thePlaylist.getPublished());
					} else { // date
						resultString = DateFormat.getDateInstance(formatConvert(dateStyle)).format(thePlaylist.getPublished());
					}
				}
	...
	public void setPublished(Date published) throws JspTagException {
		try {
			Playlist thePlaylist = (Playlist)findAncestorWithClass(this, Playlist.class);
			thePlaylist.setPublished(published);
		} catch (Exception e) {
			e.printStackTrace();
			throw new JspTagException("Error: Can't find enclosing Playlist for published tag ");
		}
